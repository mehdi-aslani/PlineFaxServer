...
namespace PlineFaxServer.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(FaxServerContext context, UserManager<TblUser> userManager,
        ...
        {
            return View(_context.Set<TblUser>());
        }
        ...
        public async Task<ActionResult> Edit(string id)
        {
            ...
            if (model == null)
            {
                HttpContext.Session.SetString(Globals.ToastError, "کاربر مورد نظر یافت نشد.");
        ...
        public async Task<ActionResult> Delete(string id)
        {
            try
            {
                ...
                var cntRec = await _context.TblReceiveFax.Where(m => m.UserId == user.Result).CountAsync();
                var cntSend = await _context.TblSendFax.Where(m => m.UserId == user.Result).CountAsync();
                var cntContact = await _context.TblContact.Where(m => m.UserId == user.Result).CountAsync();
                if (cntRec > 0 || cntSend > 0 || cntContact > 0)
                {
                    return Json(new
                    {
                        error =
                            "این کاربر در سیسم گردش کار دارد. امکان حذف این کاربر وجود ندارد. شما میتوانید این کاربر را غیر فعال نمایید."
                    });
                }
            ...
            if (ModelState.IsValid)
            {
                ...
                else if (result.Succeeded)
                {
                    ...
                    if (user.Enable)
                    {
                        ...
                        {
                            HttpContext.Session.SetString(Globals.ToastWarning,
                                "شما در حال استفاده از کلمه عبور پیش فرض می باشید. جهت ارتقای امنیت لطفا نسبت به تغییر کلمه عبور اقدام نمایید.");
